#include <cstdlib>
#include <stack>
#include <time.h>

class RouletteBet {
	float amount;
	int player_id;
};

class RoulettePlayer {
	float money;
	int player_id;
};

class Roulette {
	public:
		Roulette(int set_minimum_bet, int set_maximum_bet)
		{
			get_player_id = 1;
			minimum_bet = set_minimum_bet;
			maximum_bet = set_maximum_bet;
			srand((unsigned)time(0));
		}

		void spin()
		{
			spin_res = rand() % 37;
			std::cout << "result: " << spin_res << "\n";
			check_results();
		}

		// void place_bet_green(float amount);
		// void place_bet_single(int number, float amount);
		// void place_bet_split();

		void place_bet_1st_half(float amount)
		{
			if (!isBetAllowed(amount))
			{
				std::cout << "bet not allowed\n";
				return;
			}
			bets_1st_half.push(amount);
		}
		// void place_bet_2nd_half(float amount);
		// void place_bet_1st_third(float amount);
		// void place_bet_2nd_third(float amount);
		// void place_bet_3rd_third(float amount);

	private:
		int get_player_id;
		int minimum_bet;
		int maximum_bet;
		int spin_res;

		std::stack<float> bets_1st_half;
		std::stack<float> bets_2nd_half;
		std::stack<float> bets_1st_third;
		std::stack<float> bets_2nd_third;
		std::stack<float> bets_3rd_third;

		bool isBetAllowed(float bet)
		{
			//substract from player
			//check player total bets
			return (bool)(bet >= minimum_bet && bet <= maximum_bet);
		}

		void check_results()
		{
			while (!bets_1st_half.empty())
			{
				float bet = bets_1st_half.top();
				if (spin_res > 0 && spin_res < 19)
					std::cout << "bet wins: " << bet * 2.0 << "\n";
				else
					std::cout << "bet loses\n";
				bets_1st_half.pop();
			}
		}
};
